...
def get_token_auth_header():
    ...
    if parts[0].lower() != "bearer":
        ...
        abort_func(401, "Authorization header must be 'Bearer token'",
                   False)
    token = parts[1]
    return token
...
def verify_decode_jwt(token):
    ...
    if rsa_key:
        try:
            ...
            abort_func(401,
                       "Wrong claims!Please check the audience and issuer.",
                       False)
        except Exception:
            abort_func(400, "Unable to parse authentication token.", False)
    abort_func(400, "Invalid Header! Unable to find the appropriate key.",
               False)
